...
- 2026-07-14: **Focus-centric layout** — `focusCentricLayout(focusNodeId)` in `crdt-graph.js` dynamically reorganizes the graph on j/k navigation. BFS with sibling expansion assigns layers (parents negative, children positive, siblings same). Focus-path centering produces straight vertical edges along the navigation path (ancestors center on their discoverer child). Subtree-aware spacing prevents child overlaps. Distance-based blend (1.0/0.6/0.2) for distant nodes. Triggered from `handleNodeNavigate` when layout algorithm is 'vertical'. Design docs: `docs/explanation/auto-layout.md`.
