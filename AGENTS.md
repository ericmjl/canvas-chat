...
- 2026-06-02: Agentic mode (`/agent` slash command) — tool-using ReAct loop. **UPDATED 2026-07-16:** The loop now runs in the frontend (`agent.js`), not the backend. The backend is a stateless LLM proxy (`/api/agent/completion`). The old backend loop (`/api/agent`, `AGENT_TOOLS`) is kept only for realtime-agent.js WebSocket compat. Design docs: `docs/designs/agentic-mode/`.
...
- 2026-06-02: **Agent mimics human behavior** — The `/agent` mode MUST reuse existing feature implementations, not build parallel pipelines. When the agent runs code, it MUST produce the same CODE node as `/code`. The agent is an orchestrator that chains existing features, not a separate system. See AGENT-REQ-010 in `docs/designs/agentic-mode/agent-loop-EARS.md` and TOOL-REQ-002 in `docs/designs/agentic-mode/tool-system-EARS.md`. **UPDATED 2026-07-16:** Agent search now uses the carousel model (see below), NOT the old REFERENCE-node fan-out.
- 2026-06-02: **Agent parent routing** — `/agent` does NOT create a HUMAN node. The agent node itself links to relevant existing canvas nodes via a fast LLM routing call (`set_parents` SSE event). **UPDATED 2026-07-16:** The ref/link_from system has been REMOVED. Search results no longer get ref labels or separate nodes. `create_note` no longer accepts `link_from`. Synthesis notes auto-link to the search node as a child (GENERATES edge); the search node's carousel results feed reply context via `resolveContextWithSearchResults`. See AGENT-REQ-009 in `docs/designs/agentic-mode/agent-loop-EARS.md`.
...
- 2026-07-16: **Agent loop moved to frontend** — The agent ReAct loop now runs in JavaScript (`agent.js`), not Python. The backend is a stateless LLM proxy (`/api/agent/completion`). Tools are auto-generated from `featureRegistry.getSlashCommandsWithMetadata()` + `BUILTIN_SLASH_COMMANDS` and dispatched to the SAME feature handlers as slash commands (via `featureRegistry.handleSlashCommand`). This eliminates the structural decoupling that caused display-model drift (the root cause of the search fan-out bug). A `respond` tool handles synthesis — the model calls `respond(content="...", context_nodes=["node-1"])` to finalize. The old backend ReAct loop (`/api/agent` SSE endpoint, `AGENT_TOOLS`, `_agent_execute_tool`, `AGENT_SYSTEM_PROMPT`) is kept only for realtime-agent.js WebSocket compat. Design docs: `docs/designs/agentic-mode/LLD.md` (rewritten). **Additional features in this PR:** thinking-token stripping (`<think>` + markdown fences), search enrichment (auto-fetch top 2 pages per search), canvas context for replies, rate-limit retry (3x backoff), agent log side drawer, edge-only-via-`createLinkedNode` (bidirectional edges impossible by construction), `MAX_AGENT_TOOL_CALLS=30`.
...
| `src/canvas_chat/static/js/plugins/agent.js`           | AgentFeature class                 | `/agent` agentic mode; **frontend ReAct loop** — auto-generates tools from feature registry + BUILTIN_SLASH_COMMANDS, dispatches to slash command handlers via `featureRegistry.handleSlashCommand`, `respond` tool for synthesis, `code` tool with auto-execution, search enrichment (auto-fetch top 2 pages), thinking-token stripping, canvas context for replies |
...
| File                                            | Purpose                            | Edit for...                                                                     |
| ----------------------------------------------- | ---------------------------------- | ------------------------------------------------------------------------------- |
| `src/canvas_chat/app.py`                        | FastAPI routes, LLM proxy          | API endpoints; `/api/agent/completion` stateless streaming LLM proxy (frontend agent loop); `/api/agent` legacy SSE ReAct loop (kept for realtime-agent.js compat); `run_structured_summarize` / `run_structured_string_list` + Pydantic outputs; `AsyncSimpleBot` + `_stream_text_deltas_async_simple_bot` for streams; `/ws/agent` WebSocket endpoint for realtime voice; LiteLLM for `drop_params`, token count, Copilot models, `supports_response_schema`, `aimage_generation` |
...
### Agent tool ↔ slash command parity

**Design invariant:** A slash command (`/search`, `/code`, etc.) and its agent-mode counterpart are **two invocation paths for the same feature**. Modifying a node type, display model, or data structure for a slash command MUST automatically propagate to agent tools — without requiring a developer to manually update a separate backend code path.

**How this is enforced (2026-07-16 rewrite):** The agent loop runs in the **frontend** (`agent.js`). Tools are auto-generated from `featureRegistry.getSlashCommandsWithMetadata()`. When the agent calls a tool, the frontend dispatches to `featureRegistry.handleSlashCommand()` — the **exact same code path** as the user typing the slash command. There is no backend tool layer. The backend is a stateless LLM proxy (`/api/agent/completion`) that does NOT execute tools. This eliminates the structural decoupling entirely — there is no second code path to drift.

**Custom tools:** The `code` tool (auto-executes Python and returns output) and `respond` tool (creates the synthesis AI node) are the only non-slash-command tools. They are defined in `agent.js buildAgentTools()`.

**When you add a new slash command:** It automatically becomes an agent tool — no changes needed. Just register it in the feature registry with `getSlashCommands()`.

**RCA (2026-07-16):** This invariant was previously violated because the agent ran as a backend Python ReAct loop with hardcoded `AGENT_TOOLS` that duplicated the frontend's feature handlers. When `/search` was redesigned from fan-out to carousel (client-side JS), the backend agent's search kept the old fan-out. The fix was to move the entire loop to the frontend, eliminating the duplication.
