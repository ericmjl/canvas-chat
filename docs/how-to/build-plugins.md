...
|------|---------------|------------------|----------|
| **JavaScript-only** | ✅ | ❌ | Custom node types, slash commands, UI interactions |
| **Python-only** | ❌ | ✅ | Backend-only functionality (e.g., file processing) |
| **Paired** | ✅ | ✅ | Features requiring both frontend UI and backend processing |
...
1. **Be Specific, Not Vague**
   - ❌ Bad: "Make a plugin for polls"
   - ✅ Good: "Create a plugin that lets users create interactive polls with voting, where users click options to vote and see vote counts update in real-time"
...
class PollNode extends BaseNode {
    ...
    getActions() {
        return [
            { id: 'add-option', label: '➕ Add Option' },
            { id: 'reset-votes', label: '🔄 Reset' },
            Actions.COPY,
        ];
    }
        ...
        await this.chat.sendMessage(
            ...
            (chunk, accumulated) => {
                ...
                this.graph.updateNode(loadingNode.id, {
                    question: `🔄 Generating... (${accumulated.length} chars)`,
                });
...
1. **Node Type**: Create a custom node type called "Poll" that displays a question and voting options.
   - The node should have an icon: 📊 (bar chart emoji)
...
6. **Actions**: The node should have these action buttons:
   - "➕ Add Option" button that opens a modal to add a new option
   - "🔄 Reset" button that clears all votes (with confirmation)
...
- **Describe the Flow**: Explain what happens from start to finish - "user types command → node appears → status updates show → results stream in → final report displayed"
...
- For JS-only: Derived from JS filename (e.g., `my-plugin.js` → `my-plugin`)
- For Python-only: Derived from Python filename (e.g., `csv_handler.py` → `csv_handler`)
...
3. **Use Map for Concurrent Operations:**
   ...
   ```javascript
   // ✅ DO: Per-instance state
   this.activeOperations = new Map();

   // ❌ DON'T: Single variable
