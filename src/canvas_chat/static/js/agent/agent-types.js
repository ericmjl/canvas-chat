 ...
 * @typedef {Object} OutputDisplay
 * @property {string} [typeLabel] - Display label (e.g., 'Reflection')
 * @property {string} [typeIcon] - Emoji icon (e.g., '🔮')
 * @property {string[]} [actions] - Action button IDs (e.g., ['reply', 'copy'])
 */
 ...
 * @property {string} from - Source node reference ('$artifact', '$source', '$branch', '$leaf', or literal node ID)
 * @property {string} to - Target node reference ('$artifact', '$source', '$branch', '$leaf', or literal node ID)
 * @property {string} [edgeType] - Edge type: 'reply', 'run_reflection', 'run_artifact', 'run_trigger', 'subagent'. Default: 'reply'
 */

/**
 * Metadata update specification for postCreate hooks.
 *
 ...
 * ```yaml
 * metadata:
 *   reflectionNodeIds: [$artifact]  # Stores artifact ID, arrays are appended
 * ```
 *
 * @typedef {Object} MetadataUpdateSpec
 * @property {string} target - Node reference ('$artifact', '$source', '$branch', '$leaf', or literal node ID)
 * @property {Object} metadata - Metadata object to merge with existing (arrays append, objects shallow-merge)
 */
 ...
 * - Updating metadata on related nodes (tracking which nodes have reflections)
 * - Building graph relationships based on conversation context
 *
 * @typedef {Object} PostCreateConfig
 * @property {EdgeSpec[]} [edges] - Edges to create after artifact creation
 * @property {MetadataUpdateSpec[]} [metadataUpdates] - Metadata updates to apply after artifact creation
 * @property {boolean} [usePathContext] - If true, resolves $branch and $leaf by tracing from source node to branch point
 */
