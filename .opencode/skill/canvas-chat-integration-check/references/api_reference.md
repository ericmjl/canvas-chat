...
- **Display-model change to a shared feature (node-protocol redesign, carousel/fan-out switch, node-type add/remove)** → **INVARIANT**: slash commands (`/search`, `/code`, etc.) and their agent-mode counterpart tools (`search_web`, `execute_code`, etc.) are **two invocation paths for the same feature**. Modifying a node type or display model for a slash command MUST automatically propagate to agent tools — the agent should produce the same node structure as the slash command, not a parallel display model. **Enforcement**: agent tools carry **display DATA** (e.g. `searchResults` JSON + `searchCarouselIndex` on the search-node instruction), and the frontend node protocol (e.g. `SearchNode` in `plugins/search-node.js`) owns rendering for BOTH paths. The backend tool (`_agent_execute_tool` + `AGENT_TOOLS` + `AGENT_SYSTEM_PROMPT` in `app.py`) should NOT decide node count/types. **When you change a slash command's display model**, grep `rg '_agent_execute_tool|AGENT_TOOLS|AGENT_SYSTEM_PROMPT' src/canvas_chat/app.py` and verify the backend tool instruction still includes the fields the (possibly changed) protocol expects — the agent backend is structurally decoupled (Python SSE → JS frontend, no shared function, no compile-time signal) and will silently keep the OLD node shape if not updated. **Concrete drift instance (2026-07-16)**: `/search` was redesigned from REFERENCE-node fan-out to a carousel drawer (client-side JS only), but `/agent`'s `search_web` tool kept emitting REFERENCE fan-out — drift was invisible until a user noticed. Also check `AGENTS.md` "Agent tool ↔ slash command parity" section for the full invariant.
